...
def create_article(request):
    ...
    missing_tags = set(tags) - \
        set(existing_tags.values_list('tag_name', flat=True))
    new_tags = [Tag(tag_name=tag_name) for tag_name in missing_tags]
    Tag.objects.bulk_create(new_tags)

    # Add all tags (existing and new) to the article
    tags_to_add = existing_tags.union(
        Tag.objects.filter(Q(tag_name__in=missing_tags)))
    article.tags.add(*tags_to_add)

    # Retrieve all existing categories
    existing_categories = ArticleCategory.objects.filter(
        Q(category_name__in=categories))

    # Create new categories for any missing categories
    missing_categories = set(
        categories) - set(existing_categories.values_list('category_name', flat=True))
    new_categories = [ArticleCategory(
        category_name=category_name) for category_name in missing_categories]
    ArticleCategory.objects.bulk_create(new_categories)

    # Add all categories (existing and new) to the article
    categories_to_add = existing_categories.union(
        ArticleCategory.objects.filter(Q(category_name__in=missing_categories)))
...
def create_comment(request):
    ...
    if parent_comment_id:
        ...
        comment = Comment(article=article, name=name,
                          body=body, parent_comment=parent_comment)
    else:
...
def latest_articles(request):
    ...
    return Response({
        "latest_articles": articles_serializers.data,
    })
...
def featured_articles(request):
    featured_articles = Article.objects.filter(
        is_featured=True).order_by('-id')[:10]
    serializer = [ArticleSerializer(
        article).data for article in featured_articles]
...
def popular_articles(request):
    popular_articles = Article.objects.filter(
        is_popular=True).order_by('-created_at')[:5]
    popular_articles_data = [ArticleSerializer(
        article).data for article in popular_articles]
